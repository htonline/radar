...
std::vector<BoxInfoCus> YoloV5CustomLayer::detect(JNIEnv *env, jobject image, float threshold, float nms_threshold) {
...
std::vector<BoxInfoCus>
YoloV5CustomLayer::decode_infer(ncnn::Mat &data, int stride, const yolocvCus::YoloSize &frame_size, int net_size, int num_classes,
...
void YoloV5CustomLayer::nms(std::vector<BoxInfoCus> &input_boxes, float NMS_THRESH) {
    std::sort(input_boxes.begin(), input_boxes.end(), [](BoxInfoCus a, BoxInfoCus b) { return a.score > b.score; });
    ...
    std::sort(input_boxes.begin(), input_boxes.end(), [](BoxInfoCus a, BoxInfoCus b) { return a.y1 < b.y1; });
    for (int i = 1; i < int(input_boxes.size()); ++i) {
